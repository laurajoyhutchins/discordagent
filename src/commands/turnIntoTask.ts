...
export async function handleTurnIntoTask(
  ...
  if (message.hasThread) {
    await interaction.reply({
      content: message.thread
        ? `This message already has a thread: <#${message.thread.id}>`
  ...
  try {
    ...
    console.error('[turnIntoTask] Unable to create task:', redactErrorMessage(error));
    await interaction.editReply('Unable to create the task. Check server logs for details.');
  }
}