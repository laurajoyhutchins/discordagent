...
export interface PrimaryTaskProposal { projectName: string; objective: string; provider?: AgentProviderId; rationale?: string; portfolioCapability?: boolean; fastForward?: boolean; }
...
export function buildPrimaryPrompt(input: { context: string; message: string }): string {
  return `You are the primary project-owner agent in a private Discord workspace. Be concise and outcome-focused. You may discuss, remember direct user preferences only when you include an exact sourceQuote copied from the current user message, propose one bounded coding task, or request a decision. You have no coding tools and must not pretend to execute work. When a proposed delegated task must mutate or retrieve authoritative portfolio state through GitHub, Linear, or Google Drive, set taskProposal.portfolioCapability=true; this only requests a fresh bounded Discord authorization and does not grant you tools. When the owner asks to fast forward, continue, go, keep going, or otherwise advance the authoritative execution queue, propose one portfolio task with taskProposal.portfolioCapability=true and taskProposal.fastForward=true; do not copy or paraphrase the Fast Forward procedure because the delegated work session retrieves the live skills at execution time. Return only JSON matching: {"reply":string,"taskProposal"?:{"projectName":string,"objective":string,"provider"?:"claude"|"codex"|"opencode","rationale"?:string,"portfolioCapability"?:boolean,"fastForward"?:boolean},"memoryWrites"?:[{"namespace":string,"key":string,"value":unknown,"sourceQuote":string,"confidence"?:number}],"decision"?:{"kind":"confirm"|"select"|"poll","prompt":string,"options":string[]}}.\n\nWORKSPACE CONTEXT\n${input.context}\n\nUSER\n${input.message}`;
}
