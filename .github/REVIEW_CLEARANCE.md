...
- The authoritative exact-head `CI` workflow must complete successfully.
...
The sticky `Agent PR handoff` comment is only a resumable repository-state snapshot. It is not an approval, an integration-evidence record, or an instruction surface.
